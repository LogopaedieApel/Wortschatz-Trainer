document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadItemSet(path) { if (!path || !masterItems) return []; try { const response = await fetch(toAbsUrl(path) + '?t=' + new Date().getTime()); if (!response.ok) throw new Error(`Set-Datei nicht gefunden: ${path}`); const data = await response.json(); if (data && Array.isArray(data.items)) { return data.items; } else if (Array.isArray(data)) { return data; } else { console.error("Set-Datei hat kein 'items'-Array:", path); return []; } } catch (e) { console.error(`Fehler beim Laden oder Verarbeiten des Sets ${path}:`, e); return []; } }
    ...
    function playItemSound(item, delay = 0) { clearTimeout(soundTimeoutId); if (!item || !item.sound) { return; } const buttonToDisable = currentMode === 'quiz' ? quizSoundButton : soundButton; soundTimeoutId = setTimeout(() => { audioPlayer.pause(); audioPlayer.currentTime = 0; if (buttonToDisable) buttonToDisable.disabled = true; const enableButton = () => { if (buttonToDisable) buttonToDisable.disabled = false; audioPlayer.removeEventListener('ended', enableButton); audioPlayer.removeEventListener('error', enableButton); }; audioPlayer.addEventListener('canplay', () => audioPlayer.play().catch(e => console.error("Sound-Abspielfehler:", e)), { once: true }); audioPlayer.addEventListener('ended', enableButton, { once: true }); audioPlayer.addEventListener('error', enableButton, { once: true }); audioPlayer.src = toAbsUrl(item.sound) + '?t=' + new Date().getTime(); }, delay * 1000); }
    ...
    function populateSlide(slideElement, itemIndex) { const item = currentShuffledItems[itemIndex]; slideElement.innerHTML = ''; if (!item || item.type === 'end' || currentSettings.displayType === 'word_only') { return; } const imagePath = item.image ? item.image.trim() : ""; if (imagePath) { const img = document.createElement('img'); img.src = toAbsUrl(imagePath) + '?t=' + new Date().getTime(); img.alt = item.name || ""; if (currentMode === 'manual') { img.addEventListener('click', () => moveSlider(-1)); } slideElement.appendChild(img); } }
