document.addEventListener('DOMContentLoaded', () => {
    ...
    function populateSlide(slideElement, itemIndex) { const item = currentShuffledItems[itemIndex]; slideElement.innerHTML = ''; if (!item || item.type === 'end' || currentSettings.displayType === 'word_only') { return; } const imagePath = item.image ? item.image.trim() : ""; if (imagePath) { const img = document.createElement('img'); img.src = imagePath + '?t=' + new Date().getTime(); img.alt = item.name || ""; if (currentMode === 'manual') { img.addEventListener('click', () => moveSlider(-1)); } slideElement.appendChild(img); } }
